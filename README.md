...
- **ペルソナ忠実度ベンチマーク** — `GET /api/characters/:id/eval` がheld-out評価を実行：アーカイブから実際の（相手の発言 → キャラクターの返信）ペアを解析し、答えを隠してペルソナモデルに予測させ、予測と実際をembeddingコサイン類似度でスコアリング。平均値が **忠実度スコア（0–1）**、A–D評価。「このペルソナはどれだけ本物に近いか」への定量的で再現可能な答え。
- **非同期アーカイブ処理** — `POST /archives` は即座に `202` を返し、抽出とembeddingはキャラクター単位のバックグラウンドキューで実行。大きなアップロードがブロックしません。
- **httpOnly cookieセッション** — トークンは `localStorage` に保存されなくなりました（XSS対策）。
